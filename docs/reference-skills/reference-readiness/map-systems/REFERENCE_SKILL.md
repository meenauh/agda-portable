...
### Step 3c: Eliminate Circular Dependencies

Check for cycles in the dependency graph. If found:
- Highlight them to the user
- Resolve them before approval by changing system boundaries, extracting interface
  abstractions, splitting data/contracts from behavior, splitting UI from rules,
  or changing dependency direction.

Do not use simultaneous design as a resolution. Do not leave a cycle in the
systems index as an accepted risk. The final dependency graph must be acyclic.
Every system in the recommended design order must depend only on earlier systems
or earlier foundation contracts.

### Step 3d: Present to User

Show the dependency map as a layered list. Highlight:
- Confirmation that no circular dependencies remain
...
Pass: the dependency map summary, layer assignments, bottleneck systems list, and confirmation that the dependency graph has no cycles.
...
Before writing the systems index, verify the order as a topological order:
no system may depend on another system that appears later. If that happens,
revise the boundaries or add an earlier foundation contract before writing.
